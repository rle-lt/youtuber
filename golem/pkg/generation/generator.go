...
type VideoOutlineGenerationTemplate struct {
	Idea                          string   `json:"idea"`
	TargetAudienceCharacteristics []string `json:"targetAudienceCharacteristics"`
	ExampleTitles                 []string `json:"exampleTitles"`
	TitleStructureVariations      []string `json:"titleStructureStr"`
}
...
func (g *Generator) GenerateVideoOutlines(template VideoOutlineGenerationTemplate, promptCount uint) ([]VideoOutline, error) {
	examplesStr := strings.Join(template.ExampleTitles, "\n")
	usedTitleStr := ""
	audienceStr := ""
	titleStructureStr := ""
	...
	if err != nil {
		return []VideoOutline{}, err
	}

	messages := RequestMessages{
		{
			Role:    "system",
			Content: prompts.VIDEO_OUTLINE_GENERATION_INTRO,
		},
		BuildMessage(prompt)}
	...
	prompt = fmt.Sprintf(prompts.VIDEO_OUTLINE_TO_JSON_PROMPT, generatedPrompts)
	...
	var parsedJson struct {
		Outlines []VideoOutline `json:"outlines"`
	}

	err = json.Unmarshal([]byte(messages.GetLastMessage()), &parsedJson)
	if err != nil {
		fmt.Print(messages.GetLastMessage())
		return []VideoOutline{}, err
	}
	return parsedJson.Outlines, nil
}
