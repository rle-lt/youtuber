...
Please extract any important information from the video outline below:

<VIDEO_OUTLINE>
%s
</VIDEO_OUTLINE>

Just write down any information that wouldn't be covered in a detailed scene breakdown.
Please use the below template for formatting your response.
This would be things like instructions for scene length, overall vision, instructions for formatting, target runtime, visual style notes, etc.
...
Do NOT write the scene breakdown itself, just some extra context. Keep your responses short.
`

const STORY_ELEMENTS_PROMPT = `I'm working on writing a dramatic story for video narration based on this outline.
...
Please expand these elements into a full story structure:
...
Based on the outline provided, expand each act:
...
Expand the video outline into 12-15 detailed scenes following a 4-act dramatic structure:

**ACT 1: HOOK & SETUP (15 minutes, 3-4 scenes)**
Based on the Setup from the video outline:
...
Based on the Rising Action from the video outline:
...
REMEMBER, max limit total scenes is %d`
...
const VIDEO_OUTLINE_GENERATION_INTRO = `You are an expert at creating compelling story outlines and titles for dramatic video content. Your outlines should generate stories that hook viewers immediately and keep them engaged.`

const VIDEO_OUTLINE_GENERATION_PROMPT = `Generate %d compelling video outlines & titles for dramatic video narratives.

<CORE_THEME>
%s
</CORE_THEME>

For each outline, generate a **TITLE** with a compelling hook that makes viewers want to watch immediately
...
Each **OUTLINE** is a rough story structure with:
...
Each outline must contain enough depth, structure, and detail for an LLM to produce a complete long-form video from it
...
1. TITLE: [Your compelling title]
   OUTLINE:
   Setup: [2-3 sentences describing the opening hook and protagonist situation]
   Rising Action: [2-3 sentences on how conflict escalates and stakes increase]
   Climax: [1-2 sentences on the peak confrontation or revelation]
   Resolution: [1-2 sentences on how it resolves and the emotional payoff]
...
Continue for all %d outlines.`

const VIDEO_OUTLINE_TO_JSON_PROMPT = `You must return ONLY a valid JSON object with no additional text, markdown formatting, or code blocks.

Do not include:
- Markdown code fences 
- Any text before or after the JSON
- Comments or explanations
- List numbers in the prompt text

Required format:
{"outlines": [{"title": "...", "outline": "..."}, {"title": "...", "outline": "..."}]}

Common errors I get, make sure i don't in thus generation:
- Failed to generate title and rough outline: invalid character ']' after top-level value
